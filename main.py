...
async def start_handler(message: Message):
    ...
    await message.answer(
        ...
        "Здесь можно сканировать QR/Data Matrix, смотреть баллы и статус клиента.",
        reply_markup=main_keyboard(message.from_user.id),
    )
...
async def set_menu_button():
    try:
        await bot.set_chat_menu_button(menu_button=MenuButtonWebApp(text="QR Scanner", web_app=WebAppInfo(url=WEBAPP_URL)))
        logger.info("Menu button WebApp set successfully")
    except Exception as e:
        logger.warning("Cannot set menu button: %s", e)
