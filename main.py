...
ADMIN_WAIT_MODE = {}
ADMIN_CHECK_BALANCE_BUTTON = "🔎 Проверить баллы"
ADMIN_CHECK_STATUS_BUTTON = "🏆 Проверить статус"
...
TEXTS = {
    "ru": {
        ...
        "admin_check_balance": ADMIN_CHECK_BALANCE_BUTTON,
        "admin_check_status": ADMIN_CHECK_STATUS_BUTTON,
        "ask_username_balance": "Отправьте никнейм клиента для проверки баллов.\nНапример: <code>@username</code>",
        "ask_username_status": "Отправьте никнейм клиента для проверки статуса.\nНапример: <code>@username</code>",
...
def build_main_keyboard(lang: str, user_id: int = 0) -> ReplyKeyboardMarkup:
    keyboard = [
        [
            KeyboardButton(
    ...
    # ✅ Если заходит админ — в этой же нижней панели Telegram
    # появляются кнопки проверки баллов и статуса.
    if is_admin(user_id):
        keyboard.append([KeyboardButton(text=ADMIN_CHECK_BALANCE_BUTTON)])
        keyboard.append([KeyboardButton(text=ADMIN_CHECK_STATUS_BUTTON)])
...
def build_admin_inline_keyboard(lang: str, user_id: int = 0) -> Optional[InlineKeyboardMarkup]:
    # ✅ Дублируем админ-кнопки под сообщением /start,
    # чтобы они точно были видны даже если Telegram скрыл нижнюю клавиатуру.
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [
                InlineKeyboardButton(text=ADMIN_CHECK_BALANCE_BUTTON, callback_data="admin_check_balance"),
            ],
            [
                InlineKeyboardButton(text=ADMIN_CHECK_STATUS_BUTTON, callback_data="admin_check_status"),
            ],
        ]
    )
...
async def start_handler(message: Message):
    lang = get_user_lang(message)
    user_id = message.from_user.id if message.from_user else 0
    ensure_user_exists(user_id, lang, username=(message.from_user.username if message.from_user else ""), full_name=make_full_name(message))

    # ✅ Обновляет нижнюю Telegram-панель с кнопками.
    await message.answer(
        t(lang, "welcome"),
        reply_markup=build_main_keyboard(lang, user_id),
    )

    # ✅ Для админа дополнительно показываем панель под сообщением.
    admin_inline = build_admin_inline_keyboard(lang, user_id)
    if admin_inline:
        await message.answer(
            "👑 <b>Админ-панель</b>
...
async def admin_panel_handler(message: Message):
    ...
    await message.answer(
        "Выберите действие:",
        reply_markup=build_admin_inline_keyboard(lang, user_id),
    )
...
async def answer_client_by_username(message: Message, username: str, mode: str) -> None:
    lang = get_user_lang(message)
    username = normalize_username(username)
    ...
    try:
        ...
        logging.exception("answer client by username error: %s", e)
        await message.answer(t(lang, "internal_error"))
...
async def admin_check_balance_callback(callback):
    message = callback.message
    user_id = callback.from_user.id if callback.from_user else 0
    lang = normalize_lang(callback.from_user.language_code if callback.from_user else "ru")

    if not is_admin(user_id):
        await callback.answer("Команда только для администратора", show_alert=True)
        return

    ADMIN_WAIT_MODE[user_id] = "balance"
    await callback.answer()
    if message:
        await message.answer(t(lang, "ask_username_balance"))


@dp.callback_query(F.data == "admin_check_status")
async def admin_check_status_callback(callback):
    message = callback.message
    user_id = callback.from_user.id if callback.from_user else 0
    lang = normalize_lang(callback.from_user.language_code if callback.from_user else "ru")
    ...
    ADMIN_WAIT_MODE[user_id] = "status"
    await callback.answer()
    if message:
        await message.answer(t(lang, "ask_username_status"))


@dp.message(F.text == ADMIN_CHECK_BALANCE_BUTTON)
async def admin_check_balance_button_handler(message: Message):
    lang = get_user_lang(message)
    admin_user_id = message.from_user.id if message.from_user else 0
    update_user_profile(message, lang)
    ...
    ADMIN_WAIT_MODE[admin_user_id] = "balance"
    await message.answer(t(lang, "ask_username_balance"))


@dp.message(F.text == ADMIN_CHECK_STATUS_BUTTON)
async def admin_check_status_button_handler(message: Message):
    lang = get_user_lang(message)
    admin_user_id = message.from_user.id if message.from_user else 0
    update_user_profile(message, lang)
    ...
    ADMIN_WAIT_MODE[admin_user_id] = "status"
    await message.answer(t(lang, "ask_username_status"))


@dp.message(F.text)
async def admin_wait_username_handler(message: Message):
    lang = get_user_lang(message)
    admin_user_id = message.from_user.id if message.from_user else 0
    ...
    mode = ADMIN_WAIT_MODE.pop(admin_user_id, None)
    if not mode:
        return
    ...
    await answer_client_by_username(message, username, "status" if mode == "status" else "balance")
